...
//            writer.setPageEvent(new Watermark("重庆奔友科技有限公司"));// 水印
...
//            new PdfUtils().generatePDF(document);
     ...
     * @param fileName 文件名
     * @param list 表格内容
     * @param headColumnName 表头名
     * @param <T>
     */
    public <T> void generatePdfToFile(String fileName,List<T> list,List<String> headColumnName){
       ...
       try {
           ...
           // 4.向文档中添加内容
            new PdfUtils().generatePDF(document,list,headColumnName);
    ...
    // 生成PDF文件
    public <T> void generatePDF(Document document, List<T> list,List<String> headColumnName) throws Exception {

        // 段落
        Paragraph paragraph = new Paragraph("承运统计报表", titlefont);
...
//        table.addCell(createCell("美好的一天", headfont, Element.ALIGN_LEFT, 6, false));
        headColumnName.stream().forEach(columnName -> {
            table.addCell(createCell(columnName, keyfont, Element.ALIGN_CENTER));
        });
        ...
        list.stream().forEach( l ->{
           try {
               ...
               for (Field field : fields) {
                   ...
                   if(field.getType().getName().equals(String.class.getName()) && field.get(l)!= null){
                       table.addCell(createCell(field.get(l).toString(),textfont));
                   }
                   if(field.getType().getName().equals(BigDecimal.class.getName()) && field.get(l)!= null){
                       table.addCell(createCell(field.get(l).toString(),textfont));
                   }
               }
           }catch (Exception e){
